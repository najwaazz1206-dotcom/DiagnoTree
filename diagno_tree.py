...
def severity_label(confidence):
    """
    Menentukan label tingkat urgensi berdasarkan nilai confidence.

    Args:
        confidence (int): Nilai kepastian diagnosis.

    Returns:
        str: Label urgensi dengan warna dan ikon.
    """
    if confidence >= 90:
...
def print_result(node):
    ...
    # Label Urgensi
    print(f"  {severity_label(node.confidence)}\n")
...
def build_decision_tree():
    ...
    # DBD: Confidence 95%
    # Bintik merah (petekiae) adalah gejala patognomonik DBD
    # Sumber: Kemenkes RI (2015), Hal. 12 — Tanda Klinis DBD
    res_dbd = Node(
        text       = "Positif DBD (Demam Berdarah Dengue)",
        saran      = "Segera cek trombosit ke RS terdekat. Perbanyak minum air putih dan cairan elektrolit. Hindari aspirin atau ibuprofen.",
        confidence = 95,
        referensi  = "Kemenkes RI (2015) — Petekiae sebagai tanda patognomonik DBD",
        emoji      = "🦟"
    )

    # Suspek DBD/Flu: Confidence 75%
    # Demam tinggi + nyeri sendi tanpa petekiae masih tumpang tindih
    # Sumber: Kemenkes RI (2015), WHO Dengue Guidelines
    res_dbd_flu = Node(
        text       = "Suspek DBD atau Flu Berat",
        saran      = "Pantau kemunculan bintik merah dan suhu tubuh setiap 6 jam. Jika trombosit turun atau lemas berlebihan, segera ke IGD.",
        confidence = 75,
        referensi  = "Kemenkes RI (2015) — Kriteria suspek DBD tanpa perdarahan",
        emoji      = "⚠️"
    )

    # COVID-19: Confidence 90%
    # Anosmia/ageusia dilaporkan pada 85–91% kasus COVID-19 terkonfirmasi
    # Sumber: WHO (2022) — COVID-19 Clinical Management, Hal. 8
    res_covid = Node(
        text       = "Indikasi Kuat COVID-19",
        saran      = "Lakukan isolasi mandiri minimal 5 hari. Tes PCR atau Antigen segera. Pantau saturasi oksigen; jika < 95%, hubungi 119.",
        confidence = 90,
        referensi  = "WHO (2022) — Anosmia pada 85–91% kasus COVID-19 terkonfirmasi",
        emoji      = "🦠"
    )

    # Tifoid: Confidence 85%
    # Demam step-ladder + gejala GI adalah ciri khas tifoid
    # Sumber: Buku Ajar IPD UI Edisi VI — Demam Tifoid, Hal. 2797
    res_tipes = Node(
        text       = "Demam Tifoid (Tipes)",
        saran      = "Konsultasi dokter untuk tes Widal atau Tubex TF. Istirahat total, hindari makanan tinggi serat. Patuhi antibiotik jika diresepkan.",
        confidence = 85,
        referensi  = "Buku Ajar IPD UI Ed.VI (2014) — Demam step-ladder & gangguan GI",
        emoji      = "🌡️"
    )

    # Flu: Confidence 80%
    # Gejala ILI (Influenza-Like Illness): pilek, bersin, faringitis
    # Sumber: WHO Influenza Surveillance Guidelines (2014)
    res_flu = Node(
        text       = "Flu (Influenza / Common Cold)",
        saran      = "Istirahat cukup, perbanyak minum air hangat. Konsumsi vitamin C. Obat flu bebas (paracetamol, dekongestan) dapat membantu.",
        confidence = 80,
    ...
    # Gejala non-spesifik, belum dapat diklasifikasikan lebih lanjut
    res_demam_umum = Node(
        text       = "Demam Non-Spesifik (Demam Umum)",
        saran      = "Istirahat dan pantau perkembangan gejala dalam 24–48 jam. Kompres hangat dan paracetamol jika perlu. Ke dokter jika demam > 3 hari.",
        confidence = 60,
